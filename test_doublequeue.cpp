...
void* read_closed(void* arg) {
  ...
  num_read += 1;
  pthread_mutex_unlock(&rw_lock);
  std::cout << "l.451 in test_doublequeue.cpp" << std::endl;
  ...
  std::cout << "expect nullopt" << std::endl;
  for (int i = 0; i < 3; i++) {
    value = q->wait_remove();
    std::cout << "value.has_value(): " << value.has_value() << std::endl;
    pthread_mutex_lock(&rw_lock);
    if (value.has_value()) {
      *result = false;
      cerr << "Incorrect value from returned from call to wait_remove()";
  ...
  std::cout << "increment num_read" << std::endl;
  pthread_mutex_lock(&rw_lock);
  num_read += 1;
  pthread_mutex_unlock(&rw_lock);
